...
async function main() {
    if (window.location.href.includes('PG_QAIME_1')){
        ...
        css.rel = 'stylesheet'
        css.type = 'text/css'
        css.href = 'https://cdn.jsdelivr.net/npm/handsontable/dist/handsontable.full.min.css'
    ...
    if (!document.querySelector("#userChecker").checked){
        ...
        for (let i=0; i<htmls.length;i++){
            ...
            for (let j = 6; j <= html.querySelector('body > table > tbody').childNodes.length-2;j++){
                ...
                row.insertCell().innerHTML = [...html.querySelector("body > div:nth-child(4)").children].slice(1,-1).map(el=>el.textContent).join('')
                row.insertCell().innerHTML = html.querySelector("body > div:nth-child(4) > div:last-child").textContent
                row.insertCell().innerHTML = [...html.querySelector("body > div:nth-child(6)").children].slice(1,-1).map(el=>el.textContent).join('')
                row.insertCell().innerHTML = html.querySelector("body > div:nth-child(6) > div:last-child")?.textContent
                row.insertCell().innerHTML = html.querySelector('body > div:nth-child(3) > p:nth-child(2)')?.textContent?.slice(33)
                row.insertCell().innerHTML = html.querySelector('body > div:nth-child(3) > p:nth-child(2)')?.textContent?.slice(7,11)
                row.insertCell().innerHTML = html.querySelector('body > div:nth-child(3) > p:nth-child(2)')?.textContent?.slice(19,25)
                row.insertCell().innerHTML = html.querySelector('body > div:nth-child(8) > div')?.textContent
                row.insertCell().innerHTML = html.querySelector('body > div.mt-3.mb-3 > div')?.textContent
                row.insertCell().innerHTML = html.querySelector("body > p:nth-child(4) > span:nth-child(14)")?.textContent || ''
                row.insertCell().innerHTML = html.querySelector("body > p:nth-child(4) > span:nth-child(17)")?.textContent || ''
                row.insertCell().innerHTML = html.querySelector("body > p:nth-child(4) > span:nth-child(20)")?.textContent || ''
                row.insertCell().innerHTML = (j > 6 ? "'":'') + html.querySelector('body > table > tbody > tr:last-child > td:nth-child(11) > span')?.textContent.replace('.',',')
                row.insertCell().innerHTML = (j > 6 ? "'":'') + html.querySelector('body > table > tbody > tr:last-child > td:nth-child(16) > span')?.textContent.replace('.',',')
                row.insertCell().innerHTML = (j > 6 ? "'":'') + html.querySelector('body > table > tbody > tr:last-child > td:nth-child(17) > span')?.textContent.replace('.',',')
                row.insertCell().innerHTML = (j > 6 ? "'":'') + html.querySelector('body > table > tbody > tr:last-child > td:nth-child(18) > span')?.textContent.replace('.',',')
...
const interval = setInterval(()=>{
    let cond = false
    if (window.location.href.includes('PG_QAIME_1')){
        if (document.querySelector("#regContent")){
            cond = true
        }
    }
    if (['getDocList','getAllDocList','getDrafts'].some(x=>window.location.href.includes(x))){
        if (document?.querySelector("#accordion-control-right") && document?.querySelector("body > div.page-container > div > div.sidebar.sidebar-main") && (!document?.querySelector("#loading") || document?.querySelector("#loading")?.style.display ==='none')){
            cond = true
        }
    }
    if (window.location.href.includes('qaime.e-taxes.gov.az')){
        if (document.querySelector("#sidebarMenu")?.children?.length){
            cond = true
        }
    }
    if (window.location.href.includes('PG_REFUND')) {

        if (document.querySelector("#filterButton")){
            cond = true
        }
    }
    if (window.location.href.includes('https://www.e-taxes.gov.az/vedop2/ebyn/dispatch')){
        cond = true
    }
    if (cond){
        main()
